...
export class ProjectsService {
  ...
  getDeveloperProjects(){
    ...
    return new Promise<any>((resolve,reject)=>{
      this.afs.collection("projects").doc().collection("tasks").get().forEach((res)=>{
        res.docs.forEach(d=>{
          console.log(d);

          var obj = JSON.parse(JSON.stringify(d.data()));
          if(obj['person']==user.uid){
 ...
 getProject(id){
  ...
  (project_id, name, state) {
    return new Promise<any>((resolve, reject) => {
      this.afs.collection("projects").doc(project_id).collection("tasks").doc(name).update({
        "state": state
      })
        .then(
          res => {
