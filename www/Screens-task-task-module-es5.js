...
var TaskPage = /** @class */ (function () {
    ...
    TaskPage.prototype.ngOnInit = function () {
        ...
        this.service.getTasks(this.id).subscribe(function (doc) {
            var cache;
            _this.task = doc.map(function (element) {
                var obj = JSON.parse(JSON.stringify(element.payload.doc.data()));
                _this.service.getClient(obj.person).then(function (res) {
                    obj.person = res;
                    if (cache) {
                        if (obj.person == cache) {
                            var el = _this.totalTask[_this.totalTask.length - 1] + 1;
                            _this.totalTask.pop(_this.totalTask.length - 1);
                            _this.totalTask.push(el);
                        }
                        else {
                            _this.totalTask.push(1);
                        }
                    }
                    else {
                        _this.totalTask.push(1);
    ...
    TaskPage.prototype.calculProgres = function () {
        ...
        if (this.task.length != 0) {
            if (this.donetask.length != 0) {
                ...
                this.service.updateProject(this.id, progress).then(function (res) {
                    console.log(res);
                });
            }
        }
    ...
    TaskPage.prototype.filter = function (n) {
        ...
        if (n == 0) {
            ...
            this.task.forEach(function (element) {
                if (element.person == n) {
                    fil.push(element);
                    _this.totalhours += element.hours;
                    if (element.state != 'done') {
                        _this.lefthours += element.hours;
                    }
                }
            });
            this.creatDognut();
    ...
    TaskPage.prototype.openModify = function (id) {
        var url = "/modify-project/" + id;
        this.router.navigate([url]);
    };
    TaskPage.prototype.createLine = function () {
    ...
    TaskPage.prototype.creatDognut = function () {
