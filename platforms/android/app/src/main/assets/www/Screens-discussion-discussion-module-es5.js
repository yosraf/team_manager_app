...
var DiscussionPage = /** @class */ (function () {
    ...
    DiscussionPage.prototype.ngOnInit = function () {
        ...
        this.auth.getUsers().subscribe(function (data) {
            data.forEach(function (d) {
                ...
                if (obj['uid'] == _this.myid) {
                    ...
                    _this.sender_image = p.image;
                }
                if (obj['uid'] == _this.receiverid) {
                    var p = {
                        "image": obj.image,
                    };
                    _this.receiver_image = p.image;
                    console.log(_this.sender_image);
                }
            });
        });
        ...
        this.service.AsyncMessages(this.id).subscribe(function (res) {
            console.log(res);
            _this.messages = [];
    ...
    DiscussionPage.prototype.color = function (sender) {
        if (sender == true) {
            return '#d6b0ff';
        }
        else {
...
var ChatService = /** @class */ (function () {
    ...
    ChatService.prototype.AsyncMessages = function (user2) {
        ...
        var bool = false;
        return this.afs.collection(this.COLLECTIONMESSAGE).doc(iddoc_1).collection(this.MESSAGES, function (ref) { return ref.orderBy("created_at", 'asc'); }).snapshotChanges();
